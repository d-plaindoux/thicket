    ...
    function analyzeImports(modules, reader, imports) {
        ...
        imports.forEach(function(anImport) {
            ...
            if (!modules.contains(name)) {
                newEnvironment = newEnvironment.concat(reader.specifications(name));
            }
        });
        ...
        imports.forEach(function(anImport) {
            var name = anImport.namespace.join(".");
            if (!modules.contains(name)) {
                modules.define(anImport.namespace.join("."));
                newEnvironment = newEnvironment.concat(analyzeImports(modules, reader, reader.dependencies(name)));
            }
        });

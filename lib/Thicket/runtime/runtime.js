...
module.exports = (function() {
    ...
    Runtime.prototype.storePackage = function(aPackage) {
        return this.reader.map(function (reader) {
            reader.addPackageCode(aPackage);
        });
    };
        
    Runtime.prototype.loadPackage = function(name) {
        return this.reader.map(function (reader) {
            reader.addPackageCode(reader.packageCode(name));
        });
    };
            ...
            switch(instruction) {                   
                ...
                */                    
                ...
                case $i.code.TAILINVOKE:
                    ...
                    if (r) {                        
                        stack.unshift(r);
                        code.unshift([ $i.code.R_ANYINVOKE, v ]);
                    } else {
                ...
                case $i.code.TAILAPPLY:
                    ...
                    if (r) {                        
                        stack.unshift(r);
                        code.unshift([ $i.code.R_ANYAPPLY, v ]);
                    } else {
                ...
                case $i.code.RETURN:
                    ...
                    if (r) {                        
                        code = stack.shift().slice();
                        env = stack.shift().slice();
                ...
                case $i.code.R_NATIVE:
                    p = stack.splice(0,current[1][1]);
                    code.unshift([ $i.code.RETURN ]);
                    code = this.external[current[1][0]](p, measures).concat(code);
                    t0 = Date.now();
                    break;    

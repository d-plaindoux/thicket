...
module.exports = function () {
    ...
    Language.prototype.source = function() {
        ...
        this.parser.group("source").
            addRule([bind(this.E("importations")).to("importations"), 
                ...
                return [scope.importations, scope.entities];
            });
    };
    ...
    Language.prototype.adapter = function() {
        ...
        this.parser.group("adapterDef").
            addRule(["def",
                     ...
                     bind(this.E("type")).to('left'), "->", commit(bind(this.E("types")).to('right')), 
                     "=", 
                     bind(this.E("exprs")).to('expr')], function (scope) {
            return ast.entity(scope.name,
                              ast.type.forall(scope.generics, ast.expression(scope.name, ast.type.abstraction(scope.left, scope.right), scope.expr)));
        });
    };
    ...
    Language.prototype.types = function () {
        ...
        this.parser.group("typeDef").
            addRule(["typedef", bind(this.E('ident')).to("name"), bind(this.E("generics")).to("generics1"),
                ...
                return ast.entity(scope.name,
                                  ast.type.forall(scope.generics1, ast.typedef(scope.name, variables, ast.type.forall(scope.generics2, scope.type))));
            });
        ...
        this.parser.group("sortDef").
            addRule(["type", bind(this.E('ident')).to("name"), bind(this.E("generics")).to("generics"),
                ...
                return scope.models.map(function (model) {
                    ...
                    return ast.entity(model.name,ast.type.forall(scope.generics, model.definition));
                }).concat(ast.entity(scope.name,ast.type.forall(scope.generics, parent)));
            });
    ...
    Language.prototype.model = function () {
        ...
        this.parser.group("modelDef").
            addRule(["model", bind(this.E('ident')).to("name"), bind(this.E("generics")).to("generics"),
                     "{", commit([bind(optrep(this.E("tparam"))).to("params"), "}"])], function (scope) {
                var variables = scope.generics.map(function (name) { return ast.type.variable(name); });
                return ast.entity(scope.name,ast.type.forall(scope.generics, ast.model(scope.name, variables, scope.params)));
            }).
            addRule(["model", bind(this.E('ident')).to("name"), bind(this.E("generics")).to("generics")], function (scope) {
                var variables = scope.generics.map(function (name) { return ast.type.variable(name); });
                return ast.entity(scope.name,ast.type.forall(scope.generics, ast.model(scope.name, variables, [])));
            });
    };
    ...
    Language.prototype.expression = function () {
        ...
        this.parser.group('sexpr').
            addRule([bind(this.E("types")).to('type'), "::", commit([bind(this.E('methodName')).to('name')])], function (scope) {
                return ast.expr.abstraction("_", ast.expr.invoke(ast.expr.ident("_"),scope.name), scope.type);
            }).            
            ...
            addRule(["new", bind(this.E("exprs")).to("expr"), "with", 
                     bind(rep([this.E('ident'), "=", this.E("expr")])).to("alter")], function (scope) {
            ...
            addRule(["<", commit([bind(IDENT).to('sname'),
                                  bind(optrep([ATTRIBUTE, "=", this.E('expr')])).to('attributes'),
                ...
                return ast.expr.tag(scope.sname, 
                                    scope.attributes.map(function (l) { return [l[0], l[2]]; }), 
                                    scope.result.body || []);
            }).
        ...
        this.parser.group('tag-body').
            ...
            addRule([">", bind(optrep(this.E('expr'))).to('body'), "</", bind(IDENT).to('ename'), ">"], function (scope) {
                return scope;
            });
    };

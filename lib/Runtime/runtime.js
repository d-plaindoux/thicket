...
module.exports = (function() {
    ...
    Thicket.prototype.constant = function(t) {
        return this.executeIfLazy(t).OBJ[1].CONST;
    };
    ...
    Thicket.prototype.execute = function(toExecute, debug) {
        return this.executeCode(toExecute,[],debug);
    };
    
    Thicket.prototype.executeIfLazy = function(code, debug) {
        if (code.hasOwnProperty("LAZY_ENV")) {
            var lazyCode = code.LAZY_ENV;
            if (!lazyCode[2]) {
                lazyCode[2] = this.executeCode(lazyCode[0].LAZY.slice(), lazyCode[1].slice(), debug);
            }
    ...
    Thicket.prototype.executeCode = function(toExecute, initialEnv, debug) {
        var code = toExecute,
            env = initialEnv,
            stack = [];
        ...
        while (code.length > 0) {
            ...
            switch(getInstruction(current)) {
                ...
                case "MODEL":                      
                    ...
                    break;                    
                case "CLASS":
                    stack.unshift({OBJ:[current, this.executeIfLazy(env[0],debug)]});
                    break;                    
                case "VIEW": 
                ...
                case "INVOKE":
                    v = current.INVOKE;
                    c = this.executeIfLazy(stack.shift(),debug);
                    stack.unshift(env);
                    stack.unshift(code);
                ...
                case "RETURN":
                    ...
                    code = c;
                    env = e;
                    stack.unshift(this.executeIfLazy(v,debug));
                    break;
                case "LAZY":
        ...
        return this.executeIfLazy(stack.shift(),debug);
    };

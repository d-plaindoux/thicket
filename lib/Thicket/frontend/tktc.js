...
function main() {
    ...
    reader.orLazyElse(function () {
        console.log("[WARNING]".blue + " importation not activated (missing -i dir or -o dir option)");
    });
    
    writer.orLazyElse(function () {
        console.log("[WARNING]".blue + " compilation not activated (missing -o dir option)");
    });
    ...
    option.some(argv.p).map(function(names) {
        list(names).forEach(function(name) {
            reader.map(function(reader) {
                try {
                    reader.addPackageSpecificationAndCode(name);
                } catch (e) {
                    console.log("[ERROR]".red + " "  + ("[" + name + "]").green + " - " + e.message);
                    if (debug) {
    ...
    loadedModules = argv._.map(function(value) {
        ...
        if (result.isSuccess()) {
            ...
            return result.onFailure(function(error) {
                error.message = value.green + " - " + error.message; 
            });
        }
    });

            ...
            case 'TagExpr': 
                return list(expression.attributes).foldL(aTry.success(list()),function (result, attribute) {
                    ...
                    return list(expression.body).foldL(aTry.success(list()),function (result,body) {
                        ...
                        var creator = normalizeExpression(bindings, ast.namespace(ast.expr.ident("document"),"Client.Document")).success(),
                            tagName = normalizeExpression(bindings, ast.namespace(ast.expr.string(expression.name),"Data.String")).success(),
                            tagTerm = abstractSyntax("Apply", creator, tagName);
                            
                        tagTerm = abstractSyntax("Invoke",tagTerm,"create");
                        
                        attributes.map(function(attribute) {
                            tagTerm = abstractSyntax("Invoke",tagTerm,"addAttribute");
                            tagTerm = abstractSyntax("Apply",tagTerm,attribute[0]);
                            tagTerm = abstractSyntax("Apply",tagTerm,attribute[1]);
                        });
                        
                        body.map(function(body) {
                            tagTerm = abstractSyntax("Invoke",tagTerm,"addChild");
                            tagTerm = abstractSyntax("Apply",tagTerm,body);
                        });                                        

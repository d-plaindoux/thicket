...
exports.entities = (function () {
        ...
        switch (reflect.typeof(entity)) {
            case 'Model':
                return list(entity.params).foldL(list(), function (result, param) {
                    ...
                    return ! list(entity.generics).contains(name);
                });
            ...
            case 'View':
                return types.freeVariables(entity.param.type).filter(function (name) {
                    return ! list(entity.generics).contains(name);
                });
    ...
    Entities.prototype.analyse = function (variables, entity) {
        ...
        switch (reflect.typeof(entity)) {
            ...
            case 'View':
                newVariables = list(pair(entity.param.name, entity.param.type), pair("self", entity)).append(variables);
                return expressions.analyse(list(entity.generics), newVariables, entity.body).flatmap(function (variablesAndType) {
                    return types.unify(ast.type.native("XML"), variablesAndType._2).map(function() {
                        return entity;
                    });
                });
            default:

...
module.exports = (function () {
                ...
                if (depth) {
                    return ast.model(aType.name, list(aType.variables).map(function (variable) {
                        return that.substitute(bindings, variable, false);
                                        ...
                                        return ast.param(param.name, that.substitute(bindings, param.type, false));
                                     }).value,
                                     option.some(aType.parent).map(function (parent) {
                                        return that.substitute(bindings, parent, false);
                                     }).orElse(undefined));
                }
    ...
    Types.prototype.neutralize = function (aType) {
        ...
        // console.log("generalize(" + stringify(aType) + ") = " + stringify(instance) + " [" + instance.$type + "]");

...
exports.expressions = (function () {
    ...
    function analyseExpression(nongenerics, variables, expression) {
        switch (reflect.typeof(expression)) {
            ...
            case 'TagExpr': 
                return list(expression.attributes).foldR(function (attribute, substitutions) {
                    return substitutions.flatmap(function (substitutions) {
                        return analyseExpression(nongenerics, variables, attribute[1]).flatmap(function (variablesAndAttribute) {
                            return types.unify(variablesAndAttribute._2, ast.type.native("string")).map(function (newSubstitutions) {
                                return types.composeSubstitutions(types.composeSubstitutions(newSubstitutions, variablesAndAttribute._1), 
                                                                  substitutions);
                            });
                        });
                    });
                }, aTry.success(list())).flatmap(function (substitutions) {
                    return list(expression.body).foldR(function (body, substitutions) {
                        return substitutions.flatmap(function (substitutions) {
                            return analyseExpression(nongenerics, variables, body).flatmap(function (variablesAndBody) {
                                var newSubstitutions = types.unify(variablesAndBody._2, ast.type.native("string"));
                                if (newSubstitutions.isSuccess()) {
                                    return aTry.success(types.composeSubstitutions(types.composeSubstitutions(newSubstitutions.success(), variablesAndBody._1), 
                                                                                   substitutions));
                                }
                            
                                return types.unify(variablesAndBody._2, ast.type.native("XML")).map(function (newSubstitutions) {
                                    return types.composeSubstitutions(types.composeSubstitutions(newSubstitutions, variablesAndBody._1), 
                                                                      substitutions);
                                });                            
                            });                            
                        });
                    }, aTry.success(substitutions)).map(function (substitutions) {

...
module.exports = function () {
    ...
    Language.prototype.interpolation = function () {
        this.parser.group('interpolation').
            noSkip().
            addRule([bind(optrep(this.E("interpolation_item"))).to("interpolation"), '"', 
                     opt(SPACES)], function (scope) {
    ...
    Language.prototype.generics = function () {
        this.parser.group('generic').
            addRule(bind(this.E("ident")).to("name"), function (scope) {
                return scope.name;
            });
        
        this.parser.group('generics').
            addRule(bind(opt(["[",rep(this.E("generic"), ","),"]"])).to("generics"), function (scope) {
    ...
    Language.prototype.types = function () {
        ...
        this.parser.group("typeDef").
            addRule(["typedef", bind(this.E('ident')).to("name"), 
                     bind(this.E("generics")).to("generics1"), 
                     bind(this.E('constraints')).to("constraints1"),
                     "=", 
                     bind(this.E("generics")).to("generics2"), 
                     bind(this.E('constraints')).to("constraints2"),
                     bind(this.E("types")).to('type')], function (scope) {
                ...
                return ast.entity(scope.name,
                                  ast.type.forall(scope.generics1, 
                                                  ast.type.constraints(scope.constraints1,
                                                                       ast.typedef(scope.name, 
                                                                                   variables, 
                                                                                   ast.type.forall(scope.generics2, 
                                                                                                   ast.type.constraints(scope.constraints2,scope.type))))));
            });

        this.parser.group("sortDef").
            addRule(["type", bind(this.E('ident')).to("name"), 
                     bind(this.E("generics")).to("generics"), 
                     bind(this.E('constraints')).to("constraints"),
                ...
                return scope.models.map(function (model) {
                    ...
                    return ast.entity(model.name,ast.type.forall(scope.generics, 
                                                                 ast.type.constraints(scope.constraints,
                                                                                      ast.specialization(model.definition, variables))));
                }).concat(ast.entity(scope.name,ast.type.forall(scope.generics, 
                                                                ast.type.constraints(scope.constraints,
                                                                                     ast.specialization(parent, variables)))));
            });
    ...
    Language.prototype.definition = function() {
        ...
        this.parser.group("expressionDef").
            addRule(["def", bind(this.E('methodName')).to('name'), ":", 
                     commit([bind(this.E("generics")).to("generics"), 
                             bind(this.E('constraints')).to("constraints"),
                             bind(this.E("types")).to('type'), 
                             "=", 
                             bind(this.E("def-body")).to('expr')])], function (scope) {                
                return ast.entity(scope.name,ast.type.forall(scope.generics, 
                                                             ast.type.constraints(scope.constraints,
                                                                                  ast.expression(scope.name, scope.type, scope.expr))));
            }).
            addRule(["def", bind(this.E('methodName')).to('name'), "=", 
    ...
    Language.prototype.model = function () {
        ...
        this.parser.group("modelDef").
            addRule(["model", bind(this.E('methodName')).to("name"), 
                     bind(this.E("generics")).to("generics"), 
                     bind(this.E('constraints')).to("constraints"),                            
                     "{", commit([bind(optrep(this.E("tparam"))).to("params"), "}"])], function (scope) {
                var variables = scope.generics.map(function (name) { return ast.type.variable(name); });
            ...
            addRule(["model", bind(this.E('methodName')).to("name"), 
                     bind(this.E("generics")).to("generics"), 
                     bind(this.E('constraints')).to("constraints")], function (scope) {
                var variables = scope.generics.map(function (name) { return ast.type.variable(name); });
                return ast.entity(scope.name,ast.type.forall(scope.generics, 
                                                             ast.type.constraints(scope.constraints,
    ...
    Language.prototype.controller = function () {
        ...
        this.parser.group('traitDef').
            addRule(["trait", bind(this.E('methodName')).to('name'), 
                     bind(this.E("generics")).to("generics"),
                     bind(this.E('constraints')).to("constraints"),
                     bind(this.E("traitSpeMethods")).to('specifications'),
                     bind(this.E("traitDefMethods")).to('methods')], function (scope) {
                var variables = scope.generics.map(function (name) { return ast.type.variable(name); }),
                ...
                return ast.entity(scope.name,
                                  ast.type.forall(scope.generics, 
                                    ast.type.constraints(scope.constraints,                                                  
                                        ast.specialization(ast.trait(scope.name, 
                                                                     variables,                                                  
                                                                     specifications, 
                                                                     scope.methods || [],
                                                                     derivations),
        ...
        this.parser.group('traitSpeMethods').
            addRule(["{", commit([bind(optrep(this.E("methodSpec"))).to('specifications'), "}"])], function (scope) {
        ...
        this.parser.group('traitDefMethods').
            addRule(["{", commit([bind(optrep(this.E("methodTrait"))).to('methods'), "}"])], function (scope) {
        ...
        this.parser.group('methodTrait').
            addRule(["def", bind(this.E('methodName')).to('name'), ":", 
                ...
                return ast.method(scope.name,scope.expr,null,ast.type.forall(scope.generics, ast.type.constraints(scope.constraints,  scope.type)));
            }).
            addRule(["def", bind(this.E('methodName')).to('name'), 
        ...
        this.parser.group('controllerDef').
            addRule(["class", bind(this.E('methodName')).to('name'), 
                     bind(this.E("generics")).to("generics"),
                     bind(this.E('constraints')).to("constraints"),
    ...
    Language.prototype.param = function () {
        ...
        this.parser.group('tparam').
            addRule([bind(this.E('methodName')).to('name'), 
                     ...
                     bind(this.E('constraints')).to("constraints"),                       
                     bind(this.E("types")).to('type')], function (scope) {
                return ast.param(scope.name, ast.type.forall(scope.generics, ast.type.constraints(scope.constraints,scope.type)));
            });
    };
    ...
    Language.prototype.type = function () {
        ...
        this.parser.group('type').
            ...
            addRule([bind(this.E("generics")).to("generics"), 
                     bind(this.E('constraints')).to("constraints"),                       
                     bind(this.E("ident")).to("name"),
                     bind(opt(["from", this.E("moduleNameDef")])).to("namespace"), 
                     bind(optrep(["[", rep(this.E('type'), ","), "]"])).to('params')], function (scope) {
                var result = ast.type.variable(scope.name);
                
                if (scope.namespace.length > 0) {
                    result = ast.namespace(result, scope.namespace[0][1]);
                }

                if (scope.params.length > 0) {
                     scope.params.forEach(function(param) {
                        result = ast.type.specialize(result, param[1]);
                     });
                }

                return ast.type.forall(scope.generics, ast.type.constraints(scope.constraints,result));
            });

        this.parser.group('types').
            addRule([bind(this.E("type")).to('left'), ",", 
                     commit(bind(this.E("types")).to('right'))], function (scope) {
        ...
        this.parser.group('constraints').
            addRule(opt(["with", rep([this.E("ident"),":",this.E("type")])]), function () {
                return [];
            });            
    };
    ...
    Language.prototype.expression = function () {
        this.parser.group('exprs').
            addRule([bind(this.E("expr")).to("expr"), 
                     bind(this.E("trailingExpr")).to("trailingExpr")], function (scope) {
                return scope.trailingExpr(scope.expr);
            });

        this.parser.group("trailingExpr").
            addRule([bind(this.E("expr")).to("expr"), 
                     commit(bind(this.E("trailingExpr")).to("trailingExpr"))], function (scope) {
        ...
        this.parser.group('expr').
            addRule([bind(this.E("iexpr")).to('expr'), 
                     bind(optrep([",",commit(this.E('iexpr'))])).to('exprs')], function (scope) {
                var result = null;
        ...
        this.parser.group('iexpr').
            addRule([bind(this.E("sexpr")).to('expr'), 
                     bind(optrep([".",this.E("methodName")])).to('exprs')], function (scope) {
        ...
        this.parser.group('sexpr').
            addRule([bind(this.E("type")).to('type'), "::", 
                     commit([bind(this.E('methodName')).to('name')])], function (scope) {
            ...
            addRule([bind(this.E('operator')).to('ident'), "from", 
                     bind(this.E("moduleNameDef")).to("namespace")], function (scope) {
                return ast.namespace(ast.expr.ident(scope.ident), scope.namespace);
            }).
            addRule(bind(this.E('operator')).to('ident'), function (scope) {
                return ast.expr.ident(scope.ident);
            }).
            addRule([bind(this.E('ident')).to('ident'), "from", 
                     bind(this.E("moduleNameDef")).to("namespace")], function (scope) {
                return ast.namespace(ast.expr.ident(scope.ident), scope.namespace);
            }).
            addRule(bind(this.E('ident')).to('ident'), function (scope) {
            ...
            addRule(["let", bind(IDENT).to('ident'), 
                ...
                if (scope.hasOwnProperty("type")) {
                    variableType = ast.type.forall(scope.generics, ast.type.constraints(scope.constraints,scope.type));
                }

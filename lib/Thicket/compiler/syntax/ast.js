...
module.exports = (function () {
    ...
    //
    // Normalized expressions dedicated to Pair, Comprehension and Tag
    // 
    ...
    function tag(name, attributes, body) {
        ...
        // <A a1=v1 a2=v2> 1 </A> === document "A" create addAttribute "a1" v1 addChild 1

        newExpression = new IdentExpr("document");
        newExpression = new ApplicationExpr(newExpression, new StringExpr(name));
        newExpression = new InvokeExpr(newExpression,"create");

        attributes.map(function(attribute) {
            newExpression = new InvokeExpr(newExpression, "addAttribute");
            newExpression = new ApplicationExpr(newExpression, new StringExpr(attribute[0]));
            newExpression = new ApplicationExpr(newExpression, attribute[1]);
        });

        body.map(function(body) {
            newExpression = new InvokeExpr(newExpression, "addChild");

...
exports.language = (function () {
    ...
    function Language() {
        ...
        this.parser.group('sexpr').
            ...
            addRule(["<", commit(bind(entry('tag')).to('tag'))], function (scope) {
                return scope.tag;
            }).
        ...
        this.parser.group('tag').
            addRule([bind(IDENT).to('sname'), bind(optrep([IDENT, "=", entry('expr')])).to('attributes'), "/>"], function (scope) {
                return ast.expr.tag(scope.sname, scope.attributes.map(function (l) { return [l[0], l[2]]; }), []);
            }).
            addRule([bind(IDENT).to('sname'), bind(optrep([IDENT, "=", entry('expr')])).to('attributes'), ">",
                ...
                return ast.expr.tag(scope.sname, scope.attributes.map(function (l) { return [l[0], l[2]]; }), scope.body);
            });                          

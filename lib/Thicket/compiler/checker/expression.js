        ...
        return types.unfold(list(), environment, entity).flatmap(function (unfoldType) {
            return types.reduce(unfoldType).flatmap(function (definition) {
                ...
                switch (entity.$type) {
                    ...
                    case 'Model':
                        return list(entity.params).findFirst(function (param) {
                            ...
                            return aTry.failure(error(entity, 
                                                      "definition '" + name + "' not found in " + stringify(types.reduce(entity,true).recoverWith(entity)), 
                                                      true));
                        });
                    ...
                    case 'TypeVariable':
                        return aTry.failure(error(entity, 
                                                  "definition " + name + " not found for " + stringify(types.reduce(entity,true).recoverWith(entity))));                            
                ...
                switch (entity.$type) {
                    ...
                    case 'Model':
                        return list(entity.params).findFirst(function (param) {
                            ...
                            return aTry.failure(error(entity, 
                                                      "attribute '" + name + "' not found in " + stringify(types.reduce(entity,true).recoverWith(entity)), 
                                                      true));
                        });
                    ...
                    case 'Controller':                            
                        return aTry.failure(error(entity, 
                                                  stringify(types.reduce(entity,true).recoverWith(entity)) + " is not a model. Attribute " + name + " not found", 
                                                  true));     
                    ...
                    default: 
                        return aTry.failure(error(entity, 
                                                  "attribute " + name + " not found for " + stringify(types.reduce(entity,true).recoverWith(entity)),
                                                  true));
                }        
            });
        });

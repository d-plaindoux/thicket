...
exports.expressions = (function () {
                ...
                switch (reflect.typeof(entity)) {
                    case 'Model':
                        return list(entity.params).findFirst(function (param) {
                            ...
                            return aTry.success(pair(substitutionsAndEntity._1, types.substitute(substitutions, types.freshType(param.type))));
                    ...
                    case 'Controller':                            
                        return list(entity.specifications).findFirst(function (behavior) {
                            ...
                            return aTry.success(pair(substitutionsAndEntity._1, types.substitute(substitutions, types.freshType(behavior.type))));
                        }).orElse(aTry.failure(new Error("Accessor '" + expression.name + "' not found in " + entity)));                     
    ...
    function analysePairExpression(nongenerics, environment, substitutions, expression) {
        return analyseExpression(nongenerics, environment, substitutions, expression.left).flatmap(function (substitutionsAndLeft) {
            ...
            return analyseExpression(nongenerics, environment, newSubstitutions, expression.right).map(function (substitutionsAndRight) {
                ...
                return pair(allSubstitutions, types.substitute(substitutions, ast.type.pair(substitutionsAndLeft._2, substitutionsAndRight._2)));
            });
        });        
    }
            ...
            return analyseExpression(nongenerics, environment, newSubstitutions , expression.argument).flatmap(function (substitutionsAndArgument) {
                var newVariable = types.newVar(),
                    type1 = types.substitute(substitutions, 
                                             types.substitute(substitutionsAndArgument._1, 
                                                              types.builder(substitutionsAndAbstraction._2).recoverWith(substitutionsAndAbstraction._2))),
    ...
    function analyseComprehensionExpression(nongenerics, environment, substitutions, expression) {
        ...
        return analyseExpression(nongenerics, environment, substitutions, newExpression);
    }
    ...
    function analyseLetExpression(nongenerics, environment, substitutions, expression) {
        return analyseExpression(nongenerics, environment, substitutions, expression.value).flatmap(function (substitutionsAndValue) {
            var newEnvironment = list(pair(expression.name, types.substitute(substitutions, substitutionsAndValue._2))).append(environment);

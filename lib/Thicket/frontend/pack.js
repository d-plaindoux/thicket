...
function main(process) {
    var argv = parseArgv.usage('Usage: $0 -i dir -o dir [-v] [-d] [-s] [-n] filenames*')
                        .string("i").string("o").boolean("v").boolean("d").boolean("s").boolean("n").argv,
    ...
    argv._.map(function(specificationFile) {
        try {
            var aPackage = JSON.parse(fsreader.readContent(specificationFile)),
                specification = aPackage.definition;

...
exports.types = (function () {
            ...
            case 'Controller':
                if (depth) {
                    return ast.controller(aType.name, 
                                          ...
                                          ast.param(aType.param.name, that.substitute(bindings, aType.param.type, depth)), 
                                          list(aType.specifications).map(function (specification) {
                                             return ast.param(specification.name, that.substitute(bindings, specification.type, depth));
                                          }).value,
                                          list(aType.behaviors).map(function(behavior) {
                                             return ast.method(behavior.name, 
                                                               behavior.definition,
                                                               option.some(behavior.caller).map(function (caller) {
                                                                  return that.substitute(bindings, caller, depth);
                                                               }).orElse(undefined));
                                          }).value);
                }
            ...
            case 'Typedef':
                if (depth) {
                    return ast.typedef(aType.name, 
                                       ...
                                       that.substitute(bindings, aType.type, depth));
                }
    ...
    Types.prototype.builder = function (aType) {
        switch (reflect.typeof(aType)) {
            ...
            default:
                return aTry.failure(new Error("No builder available for type " + aType));
        }

...
function saveCode(directory, name, allEntities, entitites) {
    var destination = fs.openSync(directory + "/" + name + ".mvc.js","w");
    
    entitites.map(function (entity) {                                      
        fs.writeSync(destination, codegen.entity(list(allEntities), entity));
...
function saveSpecification(directory, name, entitites) {
    ...
    entitites.map(function (entity) {                                                       
        fs.writeSync(destination, separator);
        fs.writeSync(destination, typegen.entity(entity));
        separator = "\n, ";
    });
    fs.writeSync(destination, " ]");
    ...
    argv._.forEach(function(value) {
        ...
        if (moduleDefinitionAndEntities.isSuccess()) { 
            ...
            if (argv.o) {
                ...
                saveCode(argv.o, moduleName, allEntities, newEntities);
                saveSpecification(argv.o, moduleName, newEntities);
            }
        } else {

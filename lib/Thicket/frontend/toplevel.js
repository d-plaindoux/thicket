...
module.exports = (function() {
    ...
    Toplevel.prototype.displayError = function (error) {
        this.displayMessage("error".red + " " + error.message);
        if (this.debug) {
    ...
    Toplevel.prototype.manageSentence = function(sentence) {
        ...
        return analyzer.sentence(environment(this.packages), sentence).map(function(sentence) {
            ...
            try {        
                ...
                that.displayMessage("- : " + stringify(sentence.success().type).green + " = " + that.runtime.pretty(result));

                return atry.success(result);
            } catch (error) {
                that.displayMessage("- : " + stringify(sentence.success().type).green + " = " + "<failure>".red);
            ...
            return analyzer.entities(environment(that.packages), definitionEntitiesAndSentences[1]).map(function(subtitutionsAndEntities) {
                subtitutionsAndEntities.map(function(entity) {
                    codegen.entity(environment(that.packages), entity.definition).map(function(code) {
                        ...
                        that.displayMessage(entity.name + " : " + stringify(entity.definition).green);
                    });
                });                               
    ...
    Toplevel.prototype.loadPackage = function (name) {
        ...
        this.reader.map(function(reader) {
            try {
                var native = reader.addPackageSpecificationAndCode(reader.packageSpecificationAndCode(name));
    ...
    Toplevel.prototype.storePackage = function(content) {
        this.reader.map(function(reader) {
            reader.addPackageSpecificationAndCodeAndCode(content);
        });
    };

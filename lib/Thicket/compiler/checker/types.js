...
module.exports = (function () {
    ...
    Types.prototype.unfold = function (bindings, aType, visited) {
        if (bindings.isEmpty()) {
        ...
        switch (aType.$type) {             
            case 'TypePolymorphic':
                ...
                return that.unfold(newBindings, aType.type, visited).map(function (type) {
            ...
            case 'TypeSpecialize':
                var parameters = list(aType.parameters).foldL(aTry.success(list()), function (result, parameter) {
                    return result.flatmap(function (result) {
                        return that.unfold(bindings, parameter, visited).map(function (parameter) {
                ...
                return parameters.flatmap(function(parameters) {
                    return that.unfold(bindings,aType.type, visited).flatmap(function (type) {
                        return that.reduce(ast.type.specialize(type, parameters.value)).flatmap(function(newType) {
                            return that.unfold(bindings, newType, visited);
                        });
                    });
                });
            ...
            case 'TypeVariable':
                return bindings.findFirst(function (binding) {
                        ...
                        return that.unfold(bindings, binding._2, visited);
                    }).orElse(aTry.success(aType));
                
            case 'TypeFunction':
                return that.unfold(bindings, aType.argument).flatmap(function (argument) {
                    return that.unfold(bindings, aType.result, visited).map(function (result) {
            ...
            case 'Typedef':
                ...
                if (newVisited.contains(aType.name)) {
                    return aTry.failure(new Error("Recursive type definition for " + stringify(aType)));
                }
                
                return that.unfold(bindings, aType.type, newVisited.add(aType.name));
            ...
            case 'TypeSpecialize':
                ...
                if (genericsAndType._1.size() !== parameters.size()) {
                    return aTry.failure(error(aType,
                                              "Type " + stringify(aType) + " is waiting for " + genericsAndType._1.size() + 

...
module.exports = (function () {
    ...
    function analyseApplicationExpressionBodyFirst(varArgument, bindings, environment, substitutions, expression, type) {
        return analyseExpression(bindings, 
                                 environment,
                                 substitutions,
    ...
    function analyseApplicationExpressionArgumentFirst(varArgument, bindings, environment, substitutions, expression, type) {
        return analyseExpression(bindings, 
                                 environment,
                                 substitutions, 
                                 expression.argument, 
    ...
    function analyseApplicationExpression(bindings, environment, substitutions, expression, type) {    
        ...
        if (expression.$type === "AbstractionExpr") {
            return analyseApplicationExpressionBodyFirst(varArgument, bindings, environment, substitutions, expression, type).map(function(result) {
                return aTry.success(result);
            }).lazyRecoverWith(function() {
                return analyseApplicationExpressionArgumentFirst(varArgument, bindings, environment, substitutions, expression, type);
            });
        } else {            
            return analyseApplicationExpressionArgumentFirst(varArgument, bindings, environment, substitutions, expression, type).map(function(result) {
                return aTry.success(result);
            }).lazyRecoverWith(function() {
                return analyseApplicationExpressionBodyFirst(varArgument, bindings, environment, substitutions, expression, type);
            });
        }
    }
...
*/
        var newExpression = ast.expr.application(ast.namespace(ast.expr.ident("document"),"Client.Document"),
                                                 ast.expr.string(expression.name));
        
        newExpression = ast.expr.invoke(newExpression,"create");
        
        expression.attributes.map(function(attribute) {
            newExpression = ast.expr.invoke(newExpression, "addAttribute");
            newExpression = ast.expr.application(newExpression, ast.expr.string(attribute[0]));
            newExpression = ast.expr.application(newExpression, attribute[1]);
        });
        
        expression.body.map(function(body) {
            newExpression = ast.expr.invoke(newExpression, "addChild");
            newExpression = ast.expr.application(newExpression, body);
        });
    ...
    function analyseLetExpressionBodyFirst(varArgument, bindings, environment, substitutions, expression, type) {
        var newBindings = list(pair(expression.name, varArgument)).append(bindings);
    ...
    function analyseLetExpressionValueFirst(varArgument, bindings, environment, substitutions, expression, type) {
        return analyseExpression(bindings, 
                                 environment, 
                                 substitutions, 
                                 expression.value, 
    ...
    function analyseLetExpression(bindings, environment, substitutions, expression, type) {
        ...
        if (expression.$type === "AbstractionExpr") {
            return analyseLetExpressionBodyFirst(varArgument, bindings, environment, substitutions, expression, type).map(function(result) {
                return aTry.success(result);
            }).lazyRecoverWith(function() {
                return analyseLetExpressionValueFirst(varArgument, bindings, environment, substitutions, expression, type);
            });
        } else {
            return analyseLetExpressionValueFirst(varArgument, bindings, environment, substitutions, expression, type).map(function(result) {
                return aTry.success(result);
            }).lazyRecoverWith(function() {
                return analyseLetExpressionBodyFirst(varArgument, bindings, environment, substitutions, expression, type);
            });
        }
    }

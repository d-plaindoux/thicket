...
module.exports = (function() {
    ...
    function pretty(code ) {
        ...
        if (code instanceof Array) {
            ...
            switch (type) {
                ...
                case 'CACHED':
                    if (code.CACHED[2]) {
                        return "<CACHED " + pretty(code.CACHED[2]) + ">";
                    }

                    return "<CACHED " + pretty(code.CACHED) + ">";
                default:
                    if ('OBJ' === type) {
                        return pretty(code.OBJ);
                    }

                    if (['CLOSURE','ENV'].indexOf(type) !== -1) {
                        return "<function>";                        
                    }
                    if (type) {
                        return "<" + type + " " + pretty(code[type]) + ">";
                    } else {
    ...
    Runtime.prototype.lookupMethod = function(value, model, name) {
        ...
        for(var i = 0; entry === null && i < derivations.length; i++) {            
            ...
            if (derivation[0].CLOSURE && isClassInstance(derivation[0].CLOSURE[0])) {
                entry = this.lookupMethod(derivation[0].CLOSURE[0], model, name);
            }
        }
    ...
    Runtime.prototype.register = function(current, namespace) {
        ...
        switch(getInstruction(current)) {
            case "MODEL":
                ...
                break;
            case "CLASS":
                current.CLASS[3] = current.CLASS[2];
    ...
    Runtime.prototype.constant = function(value) {
        if (value.hasOwnProperty('CONST')) {
            ...
            return this.constant(value.OBJ[1]);
        }
            ...
            switch(instruction) {
                ...
                case "R_ANYINVOKE":
                    ...
                    if (isClassInstance(c.OBJ[0])) {
                        code = this.lookupMethodOrInternal(c.OBJ[0], c.OBJ[1], v).concat([{RETURN:1}]);
                        env = [];

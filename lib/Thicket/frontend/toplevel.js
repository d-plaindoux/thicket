...
module.exports = (function() {
        ...
        analyzer.sentence(environment(this.packages), sentence).map(function(sentence) {
            if (sentence.isFailure()) {
                ...
                try {        
                    ...
                    if (that.logAgent) {
                        try {
                            that.logAgent(stringify(sentence.success().type) + " :: " + that.runtime.pretty(result));
                        } catch (e) {
                        }
                    }
                    ...
                    if (that.logAgent) {
                        try {
                            that.logAgent(stringify(sentence.success().type) + " :: <failure>");
                        } catch (e) {
                        }
                    }
    ...
    Toplevel.prototype.manage = function(content) {        
        var that = this,
            result;
    ...
    Toplevel.prototype.loadContentAndManage = function (name) {
        var that = this;
        
        this.reader.map(function(reader) {
            that.manage(reader.content(name));
        });
    };

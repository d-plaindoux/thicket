...
module.exports = function () {
    ...
    var parser = require('../../Parser/parser.js'),
        ...
        OPERATOR = /^[#?;:@&!%><=+*/|-]+/,
        SEPARATOR = ["(", ")", "{", "}", ",", "$", "<", ">", "/>", "</", "->", "=", "//", ":", ";;"],
        KEYWORDS = ["module","from", "import", "typedef", "type", "model", "class", "view", "def", "fun", "let", "in", "if", "for"],
    ...
    Language.prototype.module = function() {
        ...
        this.parser.group("module").
            addRule(["module", bind(this.E("ident")).to("ident"), bind(optrep([".",this.E("ident")])).to("idents"), 
                     bind(this.E("source")).to("source")], function (scope) {
                var name = scope.ident;
            
                scope.idents.forEach(function (ident) {
                    name += "." + ident;
                });
            
                return [name,scope.source];
            });
    };
    ...
    Language.prototype.declarations = function() {
        ...
        this.parser.group("declarationDef").
            addRule(["from", bind(this.E("ident")).to("ident"), optrep([".",this.E("ident")]), 
                     "import", bind(this.E("importedItems")).to("names")], function (scope) {
                return ast.imports(scope.ident, scope.names);
            });
    ...
    Language.prototype.definition = function() {
        ...
        this.parser.group("expressionDef").
            addRule(["def", bind(this.E('ident')).to("name"), ":", 
                     bind(this.E("generics")).to("generics"), bind(this.E("types")).to('type'), 
                     "=", 
    ...
    Language.prototype.controller = function () {
        ...
        this.parser.group('method').
            addRule(["def", bind(opt([this.E("types"), "."])).to("caller"), bind(this.E('methodName')).to('name'), 
    ...
    Language.prototype.type = function () {
        ...
        this.parser.group('type').
            ...
            addRule(["{", bind(optrep(this.E("tparam"))).to("params"), "}"], function (scope) {
                return ast.type.structure(scope.params);
            });

...
var movicoc = require('../Movico/movicoc.js').movicoc,
    predefine = require('./predefine.js').predefine,
    M = predefine(require('./movico.js').M);
        ...
        if (chunk !== null) {
            if (endOfSource(chunk.toString())) { // `\n`
                ...
                if (sentence.isFailure()) {                    
                    ...
                    try {
                        var result = M.$$(eval(sentence.success().expression));

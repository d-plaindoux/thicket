...
function main() {
    ...
    loadedModules = argv._.map(function(value) {
        ...
        var result = thicketc.module(source);
    ...
    loadedModules.forEach(function(loadedModule) {
        loadedModule.map(function(moduleDefinitionAndEntities) {
            ...
            reader.map(function(reader) {
                allEntities = allEntities.concat(thicketc.imports(modules, reader, moduleDefinitionAndEntities[1][0]));
    ...
    loadedModules.forEach(function(loadedModule) {            
        ...
        moduleDefinitionAndEntities = thicketc.entities(allEntities, loadedModule.success()[1][1]).flatmap(function () {
            return loadedModule;
        });
        ...
        if (moduleDefinitionAndEntities.isSuccess()) { 
            ...
            writer.map(function(writer) {
                var namespace = moduleDefinitionAndEntities.success()[0],
                ...
                writer.code(moduleName, namespace, allEntities, newEntities, debug);
                writer.specification(moduleName, namespace, dependencies, newEntities, debug);
            });
        } else {

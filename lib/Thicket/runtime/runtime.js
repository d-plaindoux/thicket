...
module.exports = (function() {
    ...
    Runtime.prototype.native = function(name, arity, value) {
        ...
        if (this.delta[name]) {
            if (this.debug) {
                console.log("[Warning] Native method " + name + " aleady exist");
            }
        }
    ...
    Runtime.prototype.execute_ACCESS = function(runtime, current) {
        runtime.valueToStack(runtime.elementAtIndexFromEnv(-current[1]));
    };
    
    Runtime.prototype.execute_CLOSURE = function(runtime, current) {
    ...
    Runtime.prototype.execute_RESULT = function(runtime, current) {
    ...
    Runtime.prototype.executeNextInstruction = function(runtime, current, instruction) {
        switch(instruction) {                   
            ...
            case $i.code.ACCESS:
                this.execute_ACCESS(runtime, current);
                break;

            case $i.code.CLOSURE:
                this.execute_CLOSURE(runtime, current);
                break;
            ...
            case $i.code.RESULT:
                this.execute_RESULT(runtime, current);
                break;

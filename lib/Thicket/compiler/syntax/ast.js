...
module.exports = (function () {
    ...
    function Entity(name, definition) {
        this.$t = symbols.Entity;
        this.name = name;
        this.definition = definition;
    }
    
    function Sentence(definition) {
        this.$t = symbols.Sentence;
        this.definition = definition;
    }
    ...
    function TypePolymorphic(variables, type) {
        this.$t = symbols.TypePolymorphic;
        this.variables = variables;
        this.type = type;
    }
    
    function TypeConstraints(constraints, type) {
        this.$t = symbols.TypeConstraints;
        this.constraints = constraints;
        this.type = type;
    }

    function TypeSpecialize(type, parameters) {
        this.$t = symbols.TypeSpecialize;
        this.type = type;
        this.parameters = parameters;
    }

    function TypeNative(name) {
        this.$t = symbols.TypeNative;
        this.name = name;
    }

    function TypeVariable(name) {
        this.$t = symbols.TypeVariable;
        this.name = name;
    }

    function TypeFunction(argument,result) {
        this.$t = symbols.TypeFunction;
    ...
    function EntitySpecialization(type, parameters) {
        this.$t = symbols.EntitySpecialization;
        this.type = type;
        this.parameters = parameters;
    }
    ...
    function Method(name, caller, definition, type) {
        this.$t = symbols.Method;
        this.name = name;
        this.caller = caller;
        this.definition = definition;            
        this.type = type;
    ...
    function AbstractionExpr(param, type, body) {
        this.$t = symbols.AbstractionExpr;
        this.param = param;
        this.type = type;        
        this.body = body;            
    }
    ...
    function namespaceEntity(namespace, entity) {
        switch (entity.$t) {             
            case 'TypePolymorphic':
            case 'TypeSpecialize':
            case 'EntitySpecialization':
                namespaceEntity(namespace, entity.type);
                break;            
            case 'Model':
            case 'Trait':
            case 'Controller':  
    ...
    function namespaceEntities(namespace, entities) {
        entities.forEach(function (entity) {
            namespaceEntity(namespace, entity.definition);                
        });
    ...
    return {
        module : function (namespace, imports, entities, sentences) { return new Module(namespace, imports, namespaceEntities(namespace, entities), sentences); },
        ...
        method : function (name, expression, caller, type) { return new Method(name, caller, expression, type); },
        type: {
            forall: function (variables, type) { if (variables.length === 0) { return type; } else { return new TypePolymorphic(variables, type); } },
            constraints: function (constraints, type) { if (constraints.length === 0) { return type; } else { return new TypeConstraints(constraints, type); } },

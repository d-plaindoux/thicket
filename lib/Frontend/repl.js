...
function manageSentence(runtime, allEntities, line) {
    ...
    return sentence.map(function(sentence) {
        if (sentence.isFailure()) {
            console.log("[ERROR]".red + " " + sentence.failure().message.underline);
            return allEntities;
            ...
            if (code.isSuccess()) {
                try {        
                    ...
                    console.log("[ERROR]".red + " " + e.message.underline);
                }
            } else {
                console.log("[ERROR]".red + " " + code.failure().message);
            }
...
function manageEntities(runtime, modules, reader, allEntities, line) {        
    ...
    if (definitionAndEntities.isSuccess()) {                    
        ...
        console.log("[ERROR]".red + " - " + definitionAndEntities.failure().message);
        return newEntities;
    }
}

            ...
            case 'TypeSpecialize':
                ...
                if (genericsAndType._1.size() !== parameters.size()) {
                    return aTry.failure(error(aType.type,
                                              "Type " + stringify(this.reduce(aType.type,true).recoverWith(aType)) + " is waiting for " + genericsAndType._1.size() + 
                                              " parametric type instead of " + parameters.size()));
                }
                
                if (deep) {
                    var reducedParameters = parameters.map(function (parameter) {
                        return that.reduce(parameter, deep).recoverWith(parameter);
                    });
                    
                    return this.reduce(this.substitute(genericsAndType._1.zipWith(reducedParameters), genericsAndType._2));
                }

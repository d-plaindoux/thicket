        ...
        while (code.length !== 0) {    
            ...
            switch(instruction) {
                ...
                case $i.code.TAILINVOKE:
                    ...
                    if (r) {                        
                        stack.unshift(r);
                        code = [[ $i.code.R_ANYINVOKE, v ]].concat(code);
                    } else {
                        stack.unshift([[ $i.code.R_ANYINVOKE, v ]].concat(code), env);
                        env = c[1][1].slice();
                ...
                case $i.code.TAILAPPLY:
                    ...
                    if (r) {                        
                        stack.unshift(r);
                        code = [[ $i.code.R_ANYAPPLY, v ]].concat(code);
                    } else {
                        stack.unshift([[ $i.code.R_ANYAPPLY, v ]].concat(code), env);
                        env = c[1][1].slice();
                        code = c[1][0].concat([[ $i.code.R_CACHED, c[1] ], [ $i.code.R_RETURN, 1 ]]);
                    }                    
                    break;                          
                ...
                case $i.code.R_NATIVE:
                    p = stack.splice(0,current[1][1]);
                    code = this.external[current[1][0]](p).concat([[ $i.code.RETURN ]]).concat(code);
                    break;    

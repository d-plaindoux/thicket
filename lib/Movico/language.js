...
exports.language = (function () {
    ...
    var parser = require('../Parser/parser.js').parser,
        ...
        KEYWORDS = ["if", "for", "object", "class", "view", "let", "in"],
    ...
    function Language() {
        ...
        this.parser.group('expr').
            addRule([bind(entry("sexpr")).to('expr'), bind(entry('end_expr')).to('endExpr')], function (scope) {
                return scope.endExpr(scope.expr);
            });
        
        this.parser.group('end_expr').
            addRule([bind(/[.,]/).to('separator'), commit([bind(entry('sexpr')).to('called'), bind(entry('end_expr')).to('endExpr')])], function (scope) {
                if (scope.separator === ',') {
                    return function (expr) { return scope.endExpr(ast.expr.pair(expr, scope.called)); };
                } else {
                    return function (expr) { return scope.endExpr(ast.expr.invoke(expr, scope.called)); };
                }
            }).
            addRule([], function () {
        ...
        this.parser.group('sexpr').
            ...
            addRule(["let", bind(IDENT).to('ident'), '=', bind(entry('expr')).to('expr'), 

...
module.exports = (function() {
    ...
    Runtime.prototype.execute_NATIVE = function(runtime, current) {
        ...
        m = current[1][1];
        runtime.stack.unshift([[ $i.code.R_NATIVE, current[1] ]].concat(runtime.code), runtime.env);        

        for(n = 0; n < m; n++) {
            v = runtime.env[n];
            r = this.hasBeenEvaluated(v);                            
            if (r) {
                runtime.stack.unshift([[ $i.code.RESULT, r ],[ $i.code.R_RETURN, m-n ]], []);
            } else {
                runtime.stack.unshift(v[1][0].concat([[ $i.code.R_CACHED, v[1] ], [ $i.code.R_RETURN, m-n ]]), v[1][1].slice());
            }
        }    
        
        runtime.code = runtime.stack.shift();
        runtime.env = runtime.stack.shift();
    };
    ...
    Runtime.prototype.execute_CLASS = function(runtime, current) {
        ...
        m = 1;
        runtime.stack.unshift([[ $i.code.R_CLASS,current ]].concat(runtime.code), runtime.env);
    ...
    Runtime.prototype.execute_MODEL = function(runtime, current) {
        ...
        runtime.stack.unshift([[ $i.code.R_MODEL, [current, m] ]].concat(runtime.code), runtime.env);
    ...
    Runtime.prototype.execute_DEFINITION = function(runtime, current) {
        runtime.stack.unshift(runtime.code, runtime.env);                    
        runtime.code = current[1][1].concat([[ $i.code.RETURN ]]);
        runtime.env = [];                    
    };
    
    Runtime.prototype.execute_IDENT = function(runtime, current) {
        runtime.code = this.getIdent(current[1]).concat(runtime.code);                    
    };
    
    Runtime.prototype.execute_ALTER = function(runtime, current) {
        var n = current[1],
            m = 2,
            c = [ runtime.stack.shift(), runtime.stack.shift() ],
            v,
            r;

        runtime.stack.unshift([[ $i.code.R_ALTER, n ]].concat(runtime.code), runtime.env);
    ...
    Runtime.prototype.execute_INVOKE = function(runtime, current) {
        var v = current[1],
            c = runtime.stack.shift(), 
            r = this.hasBeenEvaluated(c);

        runtime.stack.unshift(runtime.code, runtime.env);

        if (r) {                        
            runtime.stack.unshift(r);
            runtime.code.unshift([ $i.code.R_ANYINVOKE, v ]);
        } else {
            runtime.stack.unshift([[ $i.code.R_ANYINVOKE, v ]], runtime.env);
            runtime.env = c[1][1].slice();
            runtime.code = c[1][0].concat([[ $i.code.R_CACHED, c[1] ], [ $i.code.R_RETURN, 1 ]]);
        }                    
    };

    Runtime.prototype.execute_TAILINVOKE = function(runtime, current) {
        var v = current[1],
            c = runtime.stack.shift(), 
            r = this.hasBeenEvaluated(c);
    ...
    Runtime.prototype.execute_APPLY = function(runtime) {
        var v = runtime.stack.shift(),
            c = runtime.stack.shift(),
            r = this.hasBeenEvaluated(c);                    
        ...
        if (r) {                        
            runtime.stack.unshift(r);
            runtime.code.unshift([ $i.code.R_ANYAPPLY, v ]);
        } else {
            runtime.stack.unshift([[ $i.code.R_ANYAPPLY, v ]], runtime.env);
            runtime.code = c[1][0].concat([[ $i.code.R_CACHED, c[1] ], [ $i.code.R_RETURN, 1 ]]);
            runtime.env = c[1][1].slice();
        }                    
    };
    ...
    Runtime.prototype.execute_TAILAPPLY = function(runtime) {
        var v = runtime.stack.shift(),
            c = runtime.stack.shift(),
            r = this.hasBeenEvaluated(c);                    
    ...
    Runtime.prototype.execute_RETURN = function(runtime) {
        ...
        if (r) {                        
            runtime.code = runtime.stack.shift().slice();
            runtime.env = runtime.stack.shift().slice();

            runtime.stack.unshift(r);
        } else {
            runtime.code = v[1][0].concat([[ $i.code.R_CACHED, v[1] ], [ $i.code.R_RETURN, 1 ]]);
            runtime.env = v[1][1].slice();   
        }
    };
    ...
    Runtime.prototype.execute_R_ALTER = function(runtime, current) {
        ...
        runtime.stack.unshift(this.alterValue(c,current[1],v));
    };
    
    Runtime.prototype.execute_R_MODEL = function(runtime, current) {
        var p = runtime.stack.splice(0,current[1][1]);
        
        runtime.stack.unshift([ $i.code.OBJ, [current[1][0],p] ]);  
    };
    
    Runtime.prototype.execute_R_CLASS = function(runtime, current) {
        var p = runtime.stack.shift();
        
        runtime.stack.unshift([ $i.code.OBJ, [current[1],p] ]);  
    };     
     
    Runtime.prototype.execute_R_NATIVE = function(runtime, current) {
        var p = runtime.stack.splice(0,current[1][1]);
        
        runtime.code.unshift([ $i.code.RETURN ]);
        runtime.code = this.external[current[1][0]](p, runtime.measures).concat(runtime.code);
        if (this.debug) {
            runtime.t0 = Date.now();
        }
    };
    ...
    Runtime.prototype.execute_R_ANYAPPLY = function(runtime, current) {
        var c = runtime.stack.shift();
        
        runtime.env = c[1][1].slice();
        runtime.code = c[1][0].slice();
        runtime.env.unshift(current[1]);                 
    };
    ...
    Runtime.prototype.execute_R_ANYINVOKE = function(runtime, current) {
        ...
        if (isClassInstance(c[1][0])) {
            runtime.code = this.lookupMethodOrInternal(c[1][0], c[1][1], v).concat([[ $i.code.RETURN ]]);
            runtime.env = [];
            runtime.env.unshift([ $i.code.OBJ, [c[1][0], c[1][1]] ], c[1][1]);
        } else {
            runtime.code = this.lookupAttribute(c[1][0], v).concat([[ $i.code.RETURN ]]);
            runtime.env = c[1][1].slice();
        }
    };
    
    Runtime.prototype.execute_R_RETURN = function(runtime, current) {
        var c = runtime.stack.splice(current[1],2);
    ...
    Runtime.prototype.executeNextCode = function(initialCode, initialEnv, measures) {
        var runtime = { code: initialCode, env: initialEnv, stack: [], t0: 0, measures: measures },
            current, 
            instruction;
        ...
        while (runtime.code.length !== 0) {    
            current = runtime.code.shift();
            ...
            switch(instruction) {                   
                ...
                case $i.code.ACCESS:
                    runtime.stack.unshift(runtime.env[runtime.env.length-current[1]]);                    
                    break;
                    
                case $i.code.CLOSURE:
                    runtime.stack.unshift([ $i.code.ENV, [current[1], runtime.env.slice()] ]);                    
                    break;
                ...
                case $i.code.INVOKE:
                    this.execute_INVOKE(runtime, current);
                    break;

                case $i.code.TAILINVOKE:
                    this.execute_TAILINVOKE(runtime, current);
                    break;
                    
                case $i.code.APPLY:
                    this.execute_APPLY(runtime, current);
                    break;                          

                case $i.code.TAILAPPLY:
                    this.execute_TAILAPPLY(runtime, current);
                    break;                          
                ...
                case $i.code.PUSH:
                    runtime.stack.unshift([ $i.code.CACHED, [current[1], runtime.env.slice()] ]);
                    break;
                ...
                case $i.code.RESULT:
                    runtime.stack.unshift(current[1]);
                    break;

...
module.exports = function () {
    ...
    var parser = require('../../Parser/parser.js'),
        ...
        OPERATOR = /^[#?;:@&!%><=+*/|-]+/,
        SEPARATOR = ["(", ")", "{", "}", ",", "$", "<", ">", "/>", "</", "->", "=", "//", ":", ";;"],
        KEYWORDS = ["typedef", "type", "model", "class", "view", "def", "fun", "let", "in", "if", "for"],
    ...
    Language.prototype.definition = function() {
        ...
        this.parser.group("expressionDef").
            addRule(["def", bind(this.E('ident')).to("name"), ":", bind(this.E("types")).to('type'), 
                     "=", bind(this.E("exprs")).to('expr')], function (scope) {
                return ast.expression(scope.name, scope.type, scope.expr);
            });
    };
    ...
    Language.prototype.expression = function () {
        ...
        this.parser.group('sexpr').
            ...
            addRule(["let", bind(IDENT).to('ident'), opt([":", bind(this.E("generics")).to("generics"), bind(this.E("types")).to('type')]), commit(bind(this.E("let-body")).to("body"))], function (scope) {
                var variableType;

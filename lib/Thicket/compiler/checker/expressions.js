...
module.exports = (function () {
    ...
    function unifyOrAdapt(environment, expression, providedType, type) {
        return types.unifyOrAdapt(environment, expression, providedType, type).map(function (result) {
            return aTry.success(result);
        }).lazyRecoverWith(function () {
            return aTry.failure(
                error(type, "Cannot unify " + stringify(providedType) + " and " + stringify(type))
            );
        });
    }
        ...
        return analyseExpression(bindings, environment, substitutions, expression.caller, types.newVar()).flatmap(function (substitutionsAndEntity) {
            return retrieveCallerType(environment, ast.relocate(substitute(substitutions, substitutionsAndEntity._2), expression), expression.name).flatmap(function (methodType) {
                return unifyOrAdapt(environment, 
                                    expression, 
                                    substitute(substitutionsAndEntity._1, 
                                               substitute(substitutions, methodType)), 
    ...
    function analyseLetExpression(bindings, environment, substitutions, expression, type) {
        ...
        result1 = analyseExpression(bindings, environment, substitutions, expression.value, varArgument).flatmap(function (substitutionsAndArgument) {
            return types.unfold(bindings, environment, substitute(substitutionsAndArgument._1, varArgument)).flatmap(function (unfoldType) {
                var newVarArgument = types.generalize(unfoldType),
                    newBindings = list(pair(expression.name, newVarArgument)).append(bindings);

                return analyseExpression(newBindings, environment, types.composeSubstitutions(substitutions, substitutionsAndArgument._1), expression.body, type).map(function (substitutionsAndBody) {                
        ...
        return analyseExpression(newBindings, environment, substitutions, expression.body, type).flatmap(function (substitutionsAndBody) {            
            return types.unfold(newBindings, environment, substitute(substitutionsAndBody._1, varArgument)).flatmap(function (unfoldType) {
                var newVarArgument = types.generalize(unfoldType);

                return analyseExpression(bindings, environment, types.composeSubstitutions(substitutions, substitutionsAndBody._1), expression.value, newVarArgument).map(function (substitutionsAndAbstraction) {                
    ...
    function analyseAbstractionExpression(bindings, environment, substitutions, expression, type) {
        ...
        return types.unfold(bindings, environment, varArgument).flatmap(function (varArgument) {
            return unifyOrAdapt(environment, expression, ast.type.abstraction(varArgument, varResult), type).flatmap(function (newSubstitutionsAndType) {            
                var newSubstitutions = newSubstitutionsAndType._1,
                    type = newSubstitutionsAndType._2,
    ...
    function analyseNative(environment, expression, nativeType, type) {
        return types.unifyOrAdapt(environment, expression, nativeType, type);
    }

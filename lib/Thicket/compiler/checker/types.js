...
module.exports = (function () {
    ...
    Types.prototype.public = function (aType) {
        var that = this;
                
        switch (aType.$t) {             
            case 'TypePolymorphic':
                return ast.type.forall(aType.variables, 
                                       that.public(aType.type)); 
                
            case 'TypeSpecialize':
                return ast.type.specialize(that.public(aType.type), 
                                           aType.parameters);
                
            case 'EntitySpecialization':
                return ast.specialization(that.public(aType.type), 
                                               aType.parameters);
                
            case 'TypeFunction':
                return ast.type.abstraction(that.public(aType.argument), 
                                            that.public(aType.result));
        ...
        switch (aType1.$t) {
            ...
            case 'Model':
                if (option.some(aType1.parent).isPresent()) {
                    result = that.unify(aType1.parent, aType2);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
            ...
            case 'EntitySpecialization':
                if (that.isModel(aType1.type) && option.some(aType1.type.parent).isPresent()) {
                    result = that.unify(ast.specialization(aType1.type.parent, aType1.parameters), aType2);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
        ...
        switch(aType2.$t) {        
            ...
            case 'Model':
                if (option.some(aType2.parent).isPresent()) {
                    result = that.unify(aType1, aType2.parent);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
            ...
            case 'EntitySpecialization':
                if (that.isModel(aType2.type) && option.some(aType2.type.parent).isPresent()) {
                    result = that.unify(aType1, ast.specialization(aType2.type.parent, aType2.parameters));
                    if (result.isSuccess()) {
                        return result;
                    }
                }

/*
 * Thicket
 * https://github.com/d-plaindoux/thicket
 *
 * Copyright (c) 2015 Didier Plaindoux
 * Licensed under the LGPL2 license.
 */

module.exports = (function() {
    
    "use strict";
    
    var analyzer = require('../compiler/analyzer.js'),
        codegen = require('../compiler/generator/code.js'),
        deBruijn = require('../compiler/generator/deBruijn.js'),
        objcode = require('../compiler/generator/objcode.js'),
        list = require('../../Data/list.js'),
        option = require('../../Data/option.js'),
        stringify = require('../compiler/syntax/stringify.js');    

    function codeEvaluation(code, runtime, debug) {
        return runtime.execute(code, debug);
    }

    function moduleEvaluation(reader, runtime) {
        return function(name) {
            reader.map(function(reader) {
                var binary = reader.code(name);
                binary.objcode.map(function(entity) {
                    runtime.register(entity, binary.namespace);
                });
            });
        };
    }
    
    // ------------------------------------------------------------------------------

    function Toplevel(driver, runtime, debug) {                
        this.reader = driver.map(function(driver) {
            return require('../resource/reader.js')(driver);
        });
        
        this.debug = debug;
        this.runtime = runtime;
        this.packages = require('../compiler/Data/packages.js')(option.some(moduleEvaluation(this.reader, this.runtime)));
    }

    Toplevel.prototype.manageSentence = function(allEntities, line) {
        var sentence = analyzer.sentence(allEntities, line),
            that = this;

        return sentence.map(function(sentence) {
            if (sentence.isFailure()) {
                console.log("[ERROR]" + " " + sentence.failure().message);
                if (that.debug) {
                    console.log("[STACK] " + sentence.failure().stack);
                }

                return allEntities;

            } else {
                var code = codegen.sentence(list(allEntities), sentence.success().expr);

                if (code.isSuccess()) {
                    try {        
                        var binary = objcode.generateObjCode(deBruijn.indexes(code.success())),
                            result = codeEvaluation(binary, that.runtime, that.debug);
                        console.log(stringify(sentence.success().type) + " :: " + that.runtime.pretty(result));
                    } catch (e) {
                        console.log(stringify(sentence.success().type) + " :: <failure>");
                        console.log("[ERROR]" + " " + e.message);
                        if (that.debug) {
                            console.log("[STACK] " + e.stack);
                        }
                    }
                } else {
                    console.log("[ERROR]" + " " + code.failure().message);
                    if (that.debug) {
                        console.log("[STACK] " + code.failure().stack);
                    }
                }

                return allEntities;
            }            
        });
    };

    Toplevel.prototype.manageEntities = function(allEntities, line) {        
        var newEntities = allEntities,
            that = this,
            definitionAndEntities = analyzer.source(line).flatmap(function (definitionAndEntities) {        
                that.reader.map(function(reader) {
                    newEntities = newEntities.concat(analyzer.imports(that.packages,reader,definitionAndEntities[0]));
                });

                return analyzer.entities(newEntities, definitionAndEntities[1]).map(function() {
                    return definitionAndEntities;
                });
            });

        if (definitionAndEntities.isSuccess()) {                    
            var entities = definitionAndEntities.success()[1];

            newEntities = newEntities.concat(entities);

            entities.map(function(entity) {
                codegen.entity(list(newEntities), entity).map(function(code) {
                    var binary = objcode.generateObjCode(objcode.deBruijnIndex(code));
                    that.runtime.register(binary[0]);
                });
            });

            return newEntities;
        } else {
            console.log("[ERROR]" + " - " + definitionAndEntities.failure().message);
            if (that.debug) {
                console.log("[STACK] " + definitionAndEntities.failure().stack);
            }
            return newEntities;
        }
    };
    
    Toplevel.prototype.loadAndManage = function (allEntities, name) {
        return this.manage(allEntities, this.reader.content(name));
    };

    Toplevel.prototype.manage = function(allEntities, line) {
        var that = this;
        
        return that.manageSentence(allEntities, line).orLazyElse(function() {
            return that.manageEntities(allEntities, line);
        });
    };
    
    return function (driver, runtime, debug) {
        return new Toplevel(driver, runtime, debug);
    };
    
}());

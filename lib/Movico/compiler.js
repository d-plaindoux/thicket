...
exports.compiler = (function () {
    ...
    function compileController(environment, controller) {                
        var result = list(controller.behaviors).foldL(aTry.success("'[id]':'"+ controller.name + "'"), function (result,behavior) {
            return result.flatmap(function(result) {
...
/*    
    function compileController(environment, controller) {                
        var result = list(controller.behaviors).foldL(aTry.success(list()+ controller.name + "'"), function (result,behavior) {
            return result.flatmap(function(result) {
                return compileExpression(environment, list('self',controller.param.name), behavior.definition).map(function (expression) {
                    return result.add("function(mvc$self,mvc$" + controller.param.name + "){ return " + expression ";}");
                });
            });
        });

...
module.exports = (function() {
    ...
    Runtime.prototype.executeWhenLazy = function(executionStack, code, stack, continuation) {
        var that = this;
        
        if (code.LAZY_ENV && code.LAZY_ENV.LAZY_VAL) {
            continuation(code.LAZY_ENV.LAZY_VAL);
        } else if (code.LAZY_ENV) {
            executionStack.unshift(function(executionStack) {
                code.LAZY_ENV.LAZY_VAL = stack.shift();
                continuation(code.LAZY_ENV.LAZY_VAL);
            });
            executionStack.unshift(function(executionStack) {
                that.executeNextCode(executionStack,code.LAZY_ENV[0].LAZY.slice(), code.LAZY_ENV[1].slice(),stack); 
            });
        } else {
            continuation(code);
        }
    ...
    Runtime.prototype.executeNextCode = function(executionStack,code,env,stack) {
        ...
        if (code.length > 0) {
            ...
            var current = code.shift(), n, m, v, c, e;
            ...
            switch(getInstruction(current)) {
                ...
                case "NATIVE":
                    ...
                    code = this.external[current.NATIVE[0]](env.slice(0,current.NATIVE[1])).concat([{RETURN:1}]).concat(code);
                    break;
                case "MODEL":                      
                    stack.unshift({OBJ:[current, env.slice(0,current.MODEL[1].length)]});
                    break;                    
                case "CLASS":
                    this.executeWhenLazy(executionStack, env[0], stack, function(result) {
                ...
                case "ALTER":
                    n = current.ALTER;
                    v = stack.shift();
                    this.executeWhenLazy(executionStack, stack.shift(), stack, function(m) {
                        stack.unshift(this.alterAttribute(m,n,v));
                        that.executeNextCode(executionStack,code,env,stack);
                    });
                    return;                    
                case "INVOKE":
                    ...
                    stack.unshift(env);
                    stack.unshift(code);
                    this.executeWhenLazy(executionStack, c, stack, function(c) {
                ...
                case "RETURN":
                    ...
                    code = c.slice();
                    env = e.slice();
                    this.executeWhenLazy(executionStack, v, stack, function(v) {
                        stack.unshift(v);
                        that.executeNextCode(executionStack,code,env,stack);
                    });
                    return;

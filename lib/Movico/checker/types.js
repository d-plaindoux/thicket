...
module.exports = (function () {
    ...
    Types.prototype.unfold = function (bindings, aType) {
        var that = this;
        ...
        switch (aType.$type) {             
            case 'TypePolymorphic':
                ...
                return that.unfold(newBindings, aType.type).map(function (type) {
            ...
            case 'TypeSpecialize':
                var parameters = list(aType.parameters).foldL(aTry.success(list()), function (result, parameter) {
                    return result.flatmap(function (result) {
                        return that.unfold(bindings, parameter).map(function (parameter) {
                ...
                return parameters.flatmap(function(parameters) {
                    return that.unfold(bindings,aType.type).flatmap(function (type) {
                        return that.reduce(ast.type.specialize(type, parameters.value)).flatmap(function(newType) {
                            return that.unfold(bindings, newType);
                        });
                    });
                });
            ...
            case 'TypeVariable':
                return bindings.findFirst(function (binding) {
                        ...
                        return that.unfold(bindings, binding._2);
                    }).orElse(aTry.success(aType));
                
            case 'TypeFunction':
                return that.unfold(bindings, aType.argument).flatmap(function (argument) {
                    return that.unfold(bindings, aType.result).map(function (result) {
                        return ast.type.abstraction(argument, result);
                    });
                });

            case 'Typedef':
                return that.unfold(bindings, aType.type);
    ...
    Types.prototype.unify = function(aType1, aType2) { 
        // console.log(stringify(aType1) + " =?= " + stringify(aType2));
    ...
    Types.prototype.instantiate = function (aType) {
        var that = this,
            ...
            subtitutions = genericsAndTypes._1.map(function (name) { return pair(name, that.newVar()); }),
            instance = that.substitute(subtitutions, genericsAndTypes._2);
    ...
    Types.prototype.neutralize = function (aType) {
        var that = this,
            genericsAndTypes = that.genericsAndType(aType),
            substitutions = genericsAndTypes._1.map(function (name) { return pair(name, ast.type.native(that.newName())); }),
    ...
    Types.prototype.fresh = function (aType) {
        var that = this,
            genericsAndTypes = that.genericsAndType(aType),
            substitutions = genericsAndTypes._1.map(function (name) { return pair(name, that.newVar()); }),
            instance = that.substitute(substitutions, genericsAndTypes._2);

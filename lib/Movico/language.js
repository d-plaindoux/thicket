...
exports.language = function () {
    ...
    Language.prototype.model = function () {
        ...
        this.parser.group("modelDef").
            addRule(["model", bind(this.E('ident')).to("name"), bind(this.E("generics")).to("generics"), 
                ...
                var result = ast.model(scope.name, scope.params);            
                scope.generics.reverse().forEach(function (name) {
                    result = ast.expr.forall(name, result);
                });            
                return result;
            });
    };  
    ...
    Language.prototype.controller = function () {
        ...
        this.parser.group('controllerDef').
            addRule(["class", bind(this.E('ident')).to('name'), bind(this.E("generics")).to("generics"),
                ...
                var result = ast.controller(scope.name, ast.param(scope.that, scope.type), scope.methods);
                scope.generics.reverse().forEach(function (name) {
                    result = ast.expr.forall(name, result);
                });            
                return result;
        ...
        this.parser.group('method').
            addRule(["def", bind(this.E('methodname')).to('name'), bind(optrep(this.E("ident"))).to('idents'), "=", 
        ...
        this.parser.group('methodname').
            addRule([opt([this.E("type"), "."]), bind(this.E('ident')).to('name')], function (scope) {
                return scope.name;
            });
    };
    ...
    Language.prototype.view = function () {
        ...
        this.parser.group('viewDef').
            addRule(["view", bind(this.E('ident')).to('name'), bind(this.E("generics")).to("generics"), bind(this.E('param')).to('param'),
                ...
                var result = ast.view(scope.name, scope.param, scope.body);
                scope.generics.reverse().forEach(function (name) {
                    result = ast.expr.forall(name, result);
                });            
                return result;
    ...
    Language.prototype.param = function () {    
        ...
        this.parser.group('tparam').
            addRule([bind(this.E('ident')).to('name'), ":", bind(this.E("generics")).to("generics"), bind(this.E("types")).to('type')], function (scope) {
                var result = scope.type;
                scope.generics.reverse().forEach(function (generic) {
                    result = ast.type.forall(generic, result);
                });
    ...
    Language.prototype.type = function () {
        ...
        this.parser.group('ftypes').
            addRule([bind(this.E("types")).to("type"), bind(optrep(this.E('types'))).to('params')], function (scope) {
                var result = scope.type;
                scope.params.reverse().forEach(function (type) {
        ...
        this.parser.group('types').
            addRule([bind(this.E("type")).to('left'), ",", commit(bind(this.E("ftypes")).to('right'))], function (scope) {
    ...
    Language.prototype.expression = function () {
        ...
        this.parser.group("fun-body").
            addRule(["(", ")" ,commit(["=>", bind(this.E('exprs')).to('body')])], function (scope) {
                return ast.expr.abstraction(ast.param("_",ast.type.native("unit")), scope.body);
            }).
            addRule([bind(this.E("generics")).to("generics"), bind(rep(this.E("param"))).to('params'), "=>", 
                     bind(this.E('exprs')).to('body')], function (scope) {
                var result = scope.body;
        ...
        this.parser.group('let-body').
            addRule(['=', commit([bind(this.E('exprs')).to('expr'), "in", bind(this.E('exprs')).to('body')])], function (scope) {
                return function (ident) { return ast.expr.let(ident, scope.expr, scope.body); };
            }).
            addRule(['(', commit([')', '=', bind(this.E('exprs')).to('expr'), 
                     "in", bind(this.E('exprs')).to('body')])], function (scope) {

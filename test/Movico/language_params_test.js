...
exports['language_params'] = {
  ...
  'simple typed param is accepted': function(test) {
    ...
    test.ok(language.parser.group('params').parse(aStream).isPresent(), "accept int type");
    test.done();
  },
  ...
  'simple typed param is accepted and ast params is correct': function(test) {
    ...
    test.deepEqual(language.parser.group('params').parse(aStream).get(), [ ast.param('age', ast.type()) ],  "provide one param");
    test.done();
  },
  ...
  'simple untyped param is rejected and ast params is empty': function(test) {
    ...
    test.deepEqual(language.parser.group('params').parse(aStream).get(), [],  "provide no params");
    test.done();
  },
  ...
  'mutiple typed param is accepted and ast params is correct': function(test) {
    ...
    test.deepEqual(language.parser.group('params').parse(aStream).get(), [ ast.param('age', ast.type()), ast.param('address', ast.type()) ],  "provide two params");
    test.done();
  },
};
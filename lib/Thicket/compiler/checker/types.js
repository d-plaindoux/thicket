        ...
        switch(aType2.$t) {        
            ...
            case 'Model':
                if (option.some(aType2.parent).isPresent()) {
                    result = that._unify_(aType1, aType2.parent, environment);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
            ...
            case 'EntitySpecialization':
                if (that.isModel(aType2.type) && option.some(aType2.type.parent).isPresent()) {
                    result = that._unify_(aType1, ast.specialization(aType2.type.parent, aType2.parameters), environment);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
...
*/
        ...
        return this._unify_(aType1, aType2, environment).map(function(result) {
            // console.log("<OK>");
            return result;
        });
    };    

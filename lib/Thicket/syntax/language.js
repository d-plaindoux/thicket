...
module.exports = function () {
    ...
    var parser = require('../../Parser/parser.js'),
        ...
        OPERATOR = /^([$#?;:@&!%><=+*/|-]|\[|\])([$#?;:@&!%><=+*/|_-]|\[|\])*/,
        SEPARATOR = [".", "(", ")", "{", "}", ",", "$", "<", ">", "/>", "</", "<-", "->", "=", "//", ":", ";;", "::", "$"],
        KEYWORDS = ["adapter", "module","from", "import", "typedef", "type", "model", "class", "def", "let", "in", "if", "for", "yield", "new", "with"],
    ...
    Language.prototype.adapter = function() {
        ...
        this.parser.group("adapterDef").
            ...
            return ast.type.forall(scope.generics, ast.expression(scope.name, ast.type.abstraction(scope.left, scope.right), scope.expr));
        });
    };
    ...
    Language.prototype.expression = function () {
        ...
        this.parser.group('sexpr').
            ...
            addRule(["new", bind(this.E("exprs")).to("expr"), "with", bind(rep([this.E('ident'), "=", this.E("expr")])).to("alter")], function (scope) {
            ...
            addRule(["for",
                ...
                return ast.expr.comprehension(scope.expr,
                                              scope.iterations.reverse().map(function (l) { return [l[0], l[2]]; }),

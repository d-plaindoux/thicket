...
module.exports = (function () {
    ...
    Types.prototype.varBind = function (name, aType) {
        ...
        if (that.freeVariables(aType).contains(name)) {
            return aTry.failure(error(aType, "Cyclic type dependency " + name + " == " + aType));
        } else {
    ...
    Types.prototype.unify = function(aType1, aType2) {
        ...
        switch (aType1.$type + "*" + aType2.$type) { 
            ...
            case 'TypeStructure*TypeStructure':
                var names1 = list(aType1.params).map(function (param) { return param.name; }),
                    names2 = list(aType2.params).map(function (param) { return param.name; });
                
                if (names1.same(names2)) {
                    return names1.foldR(function (name, bindings) {
                        return bindings.flatmap(function(bindings) {
                            var param1 = list(aType1.params).findFirst(function(param) { return param.name === name; }).get(),
                                param2 = list(aType2.params).findFirst(function(param) { return param.name === name; }).get(),
                                result = that.unify(that.substitute(bindings, param1.type), that.substitute(bindings, param2.type));

                            return result.map(function(result) {
                                return that.composeSubstitutions(result, bindings);
                            });
                        });
                    }, aTry.success(list()));
                }
            ...
            default:
                return aTry.failure(new Error("Cannot unify " + aType1.$type + " and " + aType2.$type));                
        }
    };

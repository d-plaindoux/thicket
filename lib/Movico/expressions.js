...
exports.expressions = (function () {
    ...
    function analyseLetExpression(nongenerics, environment, substitutions, expression, type) {
        var valueType = option.some(expression.type).orLazyElse(function () { return types.newVar(); });

        return analyseExpression(nongenerics, environment, substitutions, expression.value, valueType).flatmap(function (substitutionsAndValue) {
    ...
    function analyseExpression(nongenerics, environment, substitutions, expression, type) {
        ...
        switch (reflect.typeof(expression)) {
            ...
            case 'ApplicationExpr':
                ...
                if (result.isFailure() && reflect.typeof(expression.argument) === 'IdentExpr') {  
                    var alternative = ast.expr.invoke(expression.abstraction, expression.argument.value);                
                        analyseExpression(nongenerics, environment, substitutions, alternative, type).map(function (newResult) {
                            result = aTry.success(newResult);
                        });
                }

...
module.exports = (function () {
    ...
    function notDeclared(namespace, name) {
        return error(null, name + " available in " + namespace + " but missing in the import", true);
    }    
        
    function notFound(namespace) {
        return error(null, "Package " + namespace + " not found", true);
    }    
    ...
    Linker.prototype.linkSentence = function(namespace, sentence) {
        var result = this.linkExpression(namespace, sentence, list(), list());
        if (result.isFailure()) {
            result.failure().locate(sentence);
        }
        return result;        
    };

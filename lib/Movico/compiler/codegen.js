...
module.exports = (function () {
    ...
    function compileView(environment, view) {
        var variables = environment.map(function (entity) { 
            ...
            result = aTry.success("'[id]':'"+ view.name + "','[this]':mvc$" + view.param.name).flatmap(function (result) {
        ...
        return result.map(function (result) {
            return "M.define('" + view.name + "',function(mvc$" + view.param.name + "){return M.view(function(mvc$self){return {" + result + "};})})";
        });
    }
    ...
    function compileDefinition(environment, definition) {        
        var variables = environment.map(function (entity) { 
            ...
            body = compileExpression(variables, list(), definition.expr);

...
module.exports = (function () {
            ...
            return types.unfold(specifications, substitute(substitutions, binding._2)).flatmap(function (unfoldType) {
                ...
                return unify(providedType, type).map(function (newSubstitutions) {
                    ...
                    return builder.entityExpression(providedType).flatmap(function (expression) {
                        return unify(expression, type).map(function (newSubstitutions) {
                            return aTry.success(pair(newSubstitutions, substitute(newSubstitutions, type)));
                        });
    ...
    function analyseApplicationExpression(nongenerics, environment, specifications, substitutions, expression, type) {
        var varArgument = types.newVar(),
            result = analyseExpression(nongenerics, environment, specifications, substitutions, expression.abstraction, ast.type.abstraction(varArgument, type)).flatmap(function (substitutionsAndAbstraction) {                
        ...
        var result2 = analyseExpression(nongenerics, environment, specifications, substitutions, expression.argument, varArgument).flatmap(function (substitutionsAndArgument) {

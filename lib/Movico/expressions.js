...
exports.expressions = (function () {
    ...
    function analyseApplicationExpression(nongenerics, environment, substitutions, expression, type) {
        var varArgument = types.newVar(),
            result = analyseExpression(nongenerics, environment, substitutions , expression.abstraction, ast.type.abstraction(varArgument, type)).map(function (substitutionsAndAbstraction) {                
                return analyseExpression(nongenerics, environment, substitutions, expression.argument, substitute(substitutionsAndAbstraction._1, varArgument)).flatmap(function (substitutionsAndArgument) {
        ...
        return analyseExpression(nongenerics, environment, substitutions, expression.argument, varArgument).flatmap(function (substitutionsAndArgument) {
            return analyseExpression(nongenerics, environment, substitutions , expression.abstraction, substitute(substitutionsAndArgument._1, ast.type.abstraction(varArgument, type))).map(function (substitutionsAndAbstraction) {                
    ...
    function analyseExpression(nongenerics, environment, substitutions, expression, type) {
        ...
        switch (reflect.typeof(expression)) {
            ...
            case 'ComprehensionExpr':
                ...
                break;
            case 'TagExpr': 
                var xmlType = getNative(substitutions, "xml");

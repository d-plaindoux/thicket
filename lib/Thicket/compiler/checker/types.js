...
module.exports = (function () {
    ...
    Types.prototype.unifyOrAdapt = function (environment, expression, providedType, expectedType) {
        var that = this,
            result = that.unify(providedType, expectedType).map(function (newSubstitutions) {
                return pair(newSubstitutions, that.substitute(newSubstitutions, providedType));
            }),
            adaptedType = ast.type.abstraction(providedType, expectedType);
        
        return environment.adapters().foldL(result, function(unification, entity) {            
            if (unification.isSuccess()) {
                return unification;
            }

            return that.unfold(list(), environment, entity.definition.type).map(function (unfoldType) {
                return that.reduce(unfoldType).flatmap(function (definition) {
                    return that.instantiate(definition);
                });
            }).flatmap(function(adapter) {
                return that.unify(adaptedType, adapter).map(function(newSubstitutions) {
                   ast.adapted(expression, ast.namespace(ast.expr.ident(entity.name),entity.namespace));
                   return pair(newSubstitutions, that.substitute(newSubstitutions, expectedType));
                });
            });
        });
    };

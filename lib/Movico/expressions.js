        ...
        switch (type.get(expression)) {
            ...
            case 'InstanceExpr':
                return analyseExpression(nongenerics, variables, ast.expr.ident(expression.name)).flatmap(function (variablesAndModel) {
                    ...
                    return list(expression.params).zipWith(list(model.params)).foldR(function (pair, result) {
                        if (result.isFailure()) {
                            return result;
                        }
                        
                        return analyseExpression(nongenerics, result.success(), pair._1).flatmap(function (variablesAndParam) {
                            return types.unify(variablesAndParam._2, pair._2).map(function (substitutions) {
                                return types.composeSubstitutions(substitutions, variablesAndParam._1);
                            });
                        });                        
                    }, aTry.success(variables)).map(function (substitutions) {
                        return pair(substitutions, variablesAndModel._2);                        
                    });                                            
                });    

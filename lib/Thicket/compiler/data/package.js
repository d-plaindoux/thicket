    ...
    Package.prototype.entityExpression = function(aType) {
        switch (aType.$type) {
            ...
            case 'Model':
                ...
                }, ast.type.specialize(ast.type.forall(aType.variables.map(function(variable){ return variable.name; }),aType), 
                                       aType.variables)));
        ...
        switch (aType.$type) {
            ...
            case 'Model':
                if (aType.parent && !allowModels) {
                    return aTry.failure(error(aType, 
                                              "Model type " + stringify(aType.parent) + 
                                              " must be used instead of " + stringify(aType)));
                }

...
module.exports = (function () {
    ...
    /**
     * This method checks the trait definition. 
     * 
     * Note: method overloading is valid iff the new type is a subtype of the previous one. 
     */
    ...
    /**
     * This method computes the type of the parameter type commonly named 'this'.
     * This is required when the denoted type is a set of models.
     */
    function controllerMethodCaller(environment, paramType, method) {
        ...
        return callerType.flatmap(function (callerType) {                        
            return unify(method, callerType, paramType, environment).map(function (newSubstitutions) {
                return types.substitute(newSubstitutions, callerType);
            });
        });   
    }
                ...
                return result.flatmap(function() {
                    return expression.retrieveBehaviorType(environment, anEntity, method.name).flatmap(function (specification) {
                        return types.unfold(list(), environment, anEntity.param.type).flatmap(function (paramType) {
                            return controllerMethodCaller(environment, paramType, method).map(function (callerType) {                        

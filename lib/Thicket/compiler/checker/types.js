...
/*
        console.log("N>> " + stringify(aType1) + 
                    " <? " + stringify(aType2));

        console.log("R>> " + aType1.$t + " ?= " + aType2.$t);
*/
                ...
                if (aType1.namespace) {
                    if (environment) {
                        return environment.getType(aType1.namespace, aType1.name).flatmap(function(aType1) {
                            return that.unify(aType1, aType2, environment);
                        });
                    }
                } else {
        ...
        switch (aType1.$t) {
            case 'Typedef':
            case 'Expression':
                return that.unify(aType1.type, aType2, environment);
        }
        switch (aType2.$t) {
            case 'Typedef':
            case 'Expression':
                return that.unify(aType1, aType2.type, environment);
        }
        ...
        switch (aType1.$t + "*" + aType2.$t) {                                        
            ...
            case 'TypePolymorphic*TypePolymorphic':
                if (aType1.variables.length === aType2.variables.length) {
                    ...
                    return that.unify(aType1.type, that.substitute(bindings, aType2.type), environment);
                }
            ...
            case 'TypeFunction*TypeFunction':
                ...
                return that.unify(aType2.argument, aType1.argument, environment).flatmap(function (bindingsArgument) {
                    var aResult1 = that.substitute(bindingsArgument, aType1.result),
                        aResult2 = that.substitute(bindingsArgument, aType2.result);
                    return that.unify(aResult1, aResult2, environment).map(function (bindingsResult) {
                ...
                if (aType1.type.name === aType2.type.name) {
                    return list(aType1.parameters).zipWith(list(aType2.parameters)).foldL(aTry.success(list()), function (result, pair) {
                        return result.flatmap(function (result) {
                            ...
                            return that.unify(aResult1, aResult2, environment).map(function (substitutions) {
            ...
            case 'TypeSpecialize*EntitySpecialization':
                if (aType1.type.$t === 'TypeVariable') {
                    return list(aType1.parameters).zipWith(list(aType2.parameters)).foldL(aTry.success(list()), function (result, pair) {
                        ...
                        return that.unify(aType1.type, that.generalizeEntity(aType2.type), environment).map(function (substitutions) {
                            return result.append(substitutions);
                        });
                    });
                }
            ...
            case 'EntitySpecialization*TypeSpecialize':
                if (aType2.type.$t === 'TypeVariable') {
                    return list(aType1.parameters).zipWith(list(aType2.parameters)).foldL(aTry.success(list()), function (result, pair) {
                        ...
                        return that.unify(that.generalizeEntity(aType1.type), aType2.type, environment).map(function (substitutions) {
                            return result.append(substitutions);
                        });
                    });
                }
            ...
            case 'TypeSpecialize*TypeSpecialize':
                if (aType1.type.$t === 'TypeVariable' || aType2.type.$t === 'TypeVariable') {
                    return list(aType1.parameters).zipWith(list(aType2.parameters)).foldL(aTry.success(list()), function (result, pair) {
                        return result.flatmap(function (result) {
                            var aResult1 = that.substitute(result, pair._1),
                                aResult2 = that.substitute(result, pair._2);
                        ...
                        return that.unify(aType1.type, aType2.type, environment).map(function (substitutions) {
                            return result.append(substitutions);
                        });
                    });
                }
        ...
        switch (aType1.$t) {
            ...
            case 'TypePolymorphic':
                return that.unify(that.neutralize(aType1), aType2, environment);
                
            case 'Model':
                if (option.some(aType1.parent).isPresent()) {
                    result = that.unify(aType1.parent, aType2, environment);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
            ...
            case 'EntitySpecialization':
                if (that.isModel(aType1.type) && option.some(aType1.type.parent).isPresent()) {
                    result = that.unify(ast.specialization(aType1.type.parent, aType1.parameters), aType2, environment);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
        ...
        switch(aType2.$t) {        
            ...
            case 'TypePolymorphic':
                return that.unify(aType1, that.neutralize(aType2), environment);
                                
            case 'Model':
                if (option.some(aType2.parent).isPresent()) {
                    result = that.unify(aType1, aType2.parent, environment);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
            ...
            case 'EntitySpecialization':
                if (that.isModel(aType2.type) && option.some(aType2.type.parent).isPresent()) {
                    result = that.unify(aType1, ast.specialization(aType2.type.parent, aType2.parameters), environment);
                    if (result.isSuccess()) {
                        return result;
                    }
                }
            ...
            case 'EntitySpecialization':
                if (environment && that.isControllerOrTrait(aType1.type)) {
                    ...
                    result = list(aType1.type.derivations).foldL(aTry.failure(new Error()), function(result, derivation) {
                        ...
                        return that.unify(that.substitute(bindings, derivation), aType2, environment);
                    });
            ...
            case 'Controller':
                if (environment) {
                    result = list(aType1.derivations).foldL(aTry.failure(new Error()), function(result, derivation) {
                        ...
                        return that.unify(derivation, aType2, environment);
                    });

    ...
    TypeChecker.prototype.expression = function (environment, expression) {
        ...
        switch (type.get(expression)) {
            ...
            case 'ApplicationExpr':
                return this.expression(environment, expression.abstraction).flatMap(function (abstractionType) {
                    return that.expression(environment, expression.argument).flatMap(function (argumentType) {
                        var result = ast.type.variable("'x");
                        if (that.unify(ast.type.abstraction(argumentType, result), abstractionType)) {
                            return atry.success(that.prune(result));
                        } else {

...
/*        
        console.log(("START unifying " + stringify(types.reduce(aType1,true).recoverWith(aType1)) +
                     " with " + stringify(types.reduce(aType2,true).recoverWith(aType2))).blue);
*/        
        var result = types.unify(aType1, aType2);
/*        
        if (result.isSuccess()) {
            console.log(("unify " + stringify(types.reduce(aType1,true).recoverWith(aType1)) +
                         " with " + stringify(types.reduce(aType2,true).recoverWith(aType2))).green);
        } else {
            console.log(("unify " + stringify(types.reduce(aType1,true).recoverWith(aType1)) +
                         " with " + stringify(types.reduce(aType2,true).recoverWith(aType2))).red);
        }
*/
        ...
        var adaptedType = ast.type.abstraction(providedType, expectedType),
            result = environment.adapters().foldL(initialResult, function(unification, entity) {            
                ...
                return types.unfold(list(), environment, entity.definition).map(function (unfoldType) {
                    ...
                    return unify(expression, adaptedType, adapterType).flatmap(function(newSubstitutions) {
    ...
    function analyseApplicationExpression(bindings, environment, substitutions, expression, type) {    
        ...
        if (expression.argument.$t === "AbstractionExpr") {
    ...
    function analyseLetExpression(bindings, environment, substitutions, expression, type) {
        ...
        return types.unfold(bindings, environment, argumentType).flatmap(function (argumentType) {    
            if (expression.value.$t === "AbstractionExpr") {
    ...
    function analyseExpression(bindings, environment, substitutions, expression, type) {
        switch (expression.$t) {
            ...
            case 'ApplicationExpr':
                if (expression.argument.$t === 'IdentExpr' && !expression.argument.namespace &&
            ...
            default:
                return aTry.failure(error(expression, 
                                          "Cannot typecheck an " + expression.$t, 
                                          true));
        }
    }
